...
function tambahProduk($data)
{
    ...
    $name = htmlspecialchars(trim($data["name"])); // Mengambil nama produk
    $price = intval($data["price"]); // Mengambil harga produk
    $unit = 1000; // Unit tetap, bisa disesuaikan jika perlu
    $category = intval($data["category"]); // Mengambil kategori
    $stock = intval($data["stock"]); // Mengambil stok produk
    $descriptions = htmlspecialchars(trim($data["descriptions"])); // Mengambil deskripsi produk

    // Query dengan kolom eksplisit sesuai dengan query yang kamu berikan
    $query = "INSERT INTO products (product_name, unit, price, descriptions, category_id, stock) 
              VALUES ('$name', '$unit', '$price', '$descriptions', '$category', '$stock')";
...
function tambahGaleri($data)
{
    ...
    // Ambil data nama gambar dan produk
    $name = htmlspecialchars($data["name"]);
    $photo = upload(); // Fungsi untuk mengupload gambar
    
    if (!$photo) {
        return false; // Jika tidak ada gambar yang berhasil diupload
    }
    
    // Dapatkan ID produk dari data
    $product_id = intval($data["product_id"]); // pastikan product_id ada dalam form

    // Query untuk menambahkan ke tabel products_galleries
    $query = "INSERT INTO products_galleries (photos, product_id) 
...
function tambahUser($data)
{
    ...
    mysqli_stmt_bind_param($stmt, "sssssss", $name, $email, $password, $alamat, $phone_number, $code, $roles);
...
function tambahDriver($data)
{
    ...
    $query = "INSERT INTO drivers (name_driver, email, password, phone_number, jurusan, no_pegawai)
              VALUES ('$name', '$email', '$password', '$phone_number', '$jurusan', '$noPegawai')";
...
function tambahRekening($data)
{
    ...
    // Query untuk memasukkan data ke tabel rekening_numbers
    $query = "INSERT INTO rekening_numbers (bank_name, number, rekening_name)
              VALUES ('$bank_name', '$no_rekening', '$pemilik')";
